...
if (-not (Test-Path $paths.FEATURE_DIR -PathType Container)) {
    ...
    Write-Output "Run /cx-spec.cx-spec first to create the feature structure."
    exit 1
}
...
if (-not (Test-Path $paths.IMPL_PLAN -PathType Leaf)) {
    ...
    if ($currentMode -eq "spec") {
        ...
        Write-Output "Run /cx-spec.plan first to create the implementation plan."
        exit 1
    }
...
if ($RequireTasks -and -not (Test-Path $paths.TASKS -PathType Leaf)) {
    ...
    Write-Output "Run /cx-spec.tasks first to create the task list."
    exit 1
}
