...
func NewNTLMProxyDialContext(dialer *net.Dialer, proxyURL *url.URL,
	...
	return func(ctx context.Context, network, addr string) (net.Conn, error) {
		dialProxy := func() (net.Conn, error) {
			fmt.Printf("ntlm> Will connect to proxy at " + proxyURL.Host)
...
func dialAndNegotiate(addr, proxyUsername, proxyPassword, proxyDomain string, baseDial func() (net.Conn, error)) (net.Conn, error) {
	conn, err := baseDial()
	if err != nil {
		fmt.Printf("ntlm> Could not call dial context with proxy: %s", err)
		return conn, err
	}
	// NTLM Step 1: Send Negotiate Message
	negotiateMessage, err := newNegotiateMessage(proxyDomain, "")
	if err != nil {
		fmt.Printf("ntlm> Could not negotiate domain '%s': %s", proxyDomain, err)
		return conn, err
	}
	fmt.Printf("ntlm> NTLM negotiate message: '%s'", base64.StdEncoding.EncodeToString(negotiateMessage))
	...
	fmt.Printf("ntlm> Successfully sent negotiate message to proxy")
	...
	if err != nil {
		fmt.Printf("ntlm> Could not read response body from proxy: %s", err)
		return conn, err
	}
	resp.Body.Close()
	if resp.StatusCode != http.StatusProxyAuthRequired {
		fmt.Printf("ntlm> Expected %d as return status, got: %d", http.StatusProxyAuthRequired, resp.StatusCode)
	...
	fmt.Printf("ntlm> NTLM challenge: '%s'", challenge[1])
	challengeMessage, err := base64.StdEncoding.DecodeString(challenge[1])
	if err != nil {
		fmt.Printf("ntlm> Could not base64 decode the NTLM challenge: %s", err)
		return conn, err
	}
	// NTLM Step 3: Send Authorization Message
	fmt.Printf("ntlm> Processing NTLM challenge with username '%s' and password with length %d", proxyUsername, len(proxyPassword))
	authenticateMessage, err := processChallenge(challengeMessage, proxyUsername, proxyPassword)
	if err != nil {
		fmt.Printf("ntlm> Could not process the NTLM challenge: %s", err)
		return conn, err
	}
	fmt.Printf("ntlm> NTLM authorization: '%s'", base64.StdEncoding.EncodeToString(authenticateMessage))
