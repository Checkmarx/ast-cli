...
func addDescriptionForIdentifier(responseContent []string) []string {
	identifiersDescription := map[string]string{
		confidence:  " A score between 0 (low) and 100 (high) indicating OpenAI's confidence level for the effectiveness of the suggested remediation. <br>",
		explanation: " An OpenAI generated description of the vulnerability. <br>",
		fix:         " A customized snippet, generated by OpenAI, that can be used to remediate the vulnerability in your code. <br>",
	}
	identifierTitleFormat := "<span style=\"color: regular;\">%s</span><span style=\"color: grey; font-style: italic;\">%s</span>"
	if len(responseContent) > 0 {
		for i := 0; i < len(responseContent); i++ {
			line := responseContent[i]
			for identifier, description := range identifiersDescription {
				line = strings.Replace(line, identifier, fmt.Sprintf(identifierTitleFormat, identifier, description), 1)
			}
			responseContent[i] = line
		}
	}
