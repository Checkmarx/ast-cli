...
const (
	...
	SuccessAuthValidate  = "Successfully authenticated to ONE server!"
...
func NewAuthCommand(authWrapper wrappers.AuthWrapper) *cobra.Command {
	authCmd := &cobra.Command{
		...
		Example: heredoc.Doc(
			`
			$ cx auth validate
			Successfully authenticated to ONE server!
	...
	createClientCmd := &cobra.Command{
		Use:     "register",
		Short:   "Register new OAuth2 client for ONE",
	...
	createClientCmd.PersistentFlags().StringP(
		params.UsernameFlag, params.UsernameSh, "", "Username for ONE user that privileges to "+
			"create clients",
	)
	createClientCmd.PersistentFlags().StringP(
		params.PasswordFlag, params.PasswordSh, "", "Password for ONE user that privileges to "+
			"create clients",
	)
	createClientCmd.PersistentFlags().StringP(
	...
	validLoginCmd := &cobra.Command{
		...
		Long:  "Validates if CLI is able to communicate with ONE",
		Annotations: map[string]string{
			"command:doc": heredoc.Doc(
				`
