...
func TestSastResourcesEnginesCommand(t *testing.T) {
	cmd := createASTTestCommand()
	err := executeTestCommand(cmd, "sast-resources", "engines", "--format", "pretty")
	assert.NilError(t, err)
...
func TestSastResourcesStatsCommand(t *testing.T) {
	cmd := createASTTestCommand()
	err := executeTestCommand(cmd, "sast-resources", "stats", "--format", "pretty")
	assert.NilError(t, err)
	err = executeTestCommand(cmd, "sr", "stats")
	assert.NilError(t, err)
