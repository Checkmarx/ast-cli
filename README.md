...
This document provides many examples of using the AST CLI, but it's impossible to cover every possible action. You can always fall back to the (--help or -h) option, ex:
...
- (--client-secret), the secret that corresponds to the client-id  (see Authentication documentation).
...
Many CLI variables can be provided using environment variables, configuration variables or CLI parameters. The follow precedence is used when the same value is found in settings:

1. CLI parameters, these always override configuration and environment variables.
2. Configuration variables always override environment variables.
3. Environment variables are the first order precedence.
...
The (--profile) option provides a powerful tool to quickly switch through different sets of configurations. You can add a profile name to any CLI command and it utilize the corsponding profile settings. The following example setups up altnerative profile named "test" and calls a CLI command utilizing it.
...
AST Client ID  [******f23d]: <your-updated-client-id>
...
- cx_client_secret: the secret that corresponds to the client-id  (see Authentication documentation).
...
If you are using a hosted AST instance you will need to include your tenant name when using the CLI. Failure to configure your tenant information correctly will result in communication failures with AST.
...
Once command completion has been set up you simply press [tab] [tab] to get a list of supported commands and arguments. The result you see will depend on the context when you press [tab] [tab]. The following example shows the results when using the (project list) command:
...
...todo, we're still waiting to work out result retrieval
...
The following example demonstrates the use of a proxy server:
